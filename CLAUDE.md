...
**Phase0〜Phase3 が完了し、GitHub Pages で公開されている（2026-08-22時点）。** 設計文書・サイト骨格・クイズエンジン・概念パート全6章・Phase1のデータ整備・引用の一次資料での裏取り・Rハンズオン本編3本が main に入っている。**納品前監査（2026-08-21）で起票された issue #44〜#55 の12件（監査 F-01〜F-12 対応）のうち11件はクローズ済み。** その後のコントラスト関連 issue #64・#65・#66・#68・#71 もクローズ済み。**現在 open な issue は #54（監修体制。独立監修は置かない方針は 2026-08-21 のユーザーコメントで決着済みで、残るのは承認記録）と #73（修了証（目録）機能の移植）の2件だけ。**
...
1. **監修体制** — 統計・疫学的内容の正確性を誰がレビューするか（[要件定義書](documents/要件定義書.md) §9）
