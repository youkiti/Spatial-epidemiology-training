...
概念パート(全6章)は、章1から章6まで順番に読み進める構成です。章2「空間重み行列」は章1(記述)と章3以降(空間統計)の橋渡しにあたり、章4「LISA / Gi\* / SaTScan の違い」がこの教材の山場です。各章はまず事前テストを解いてから本文に入ります。Rハンズオンは対応する概念パートの章を読んだあとに取り組むと理解しやすい構成にしています。

## クイズの位置づけ

各章には2種類のクイズがあります。

- **事前テスト**(各章3問) — 合否を記録しません。章の冒頭(学習目標の直後)にあり、本文を読む前に解いて理解の現在地を確認するためのものです(正解できなくて構いません)。
