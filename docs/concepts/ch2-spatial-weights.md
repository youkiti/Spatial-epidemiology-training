...
ではなぜ本教材のハンズオンは、この1行をその場で呼ぶのではなく、あらかじめ計算済みの `data/geo/adjacency_iryoken2.csv`(列は `area_code` / `neighbor_code` のエッジ一覧)を読み込む構成になっているのでしょうか。理由は2つありますが、主な理由は教材としての方針です。隣接の定義をコミット済みの成果物として固定しておくことは、本章の主題である「隣を先に決める」という考え方をそのまま体現しています。`poly2nb()` 自体は同じポリゴンと同じオプションを渡せば常に同じ結果を返す決定的な関数ですが、隣接をその場で計算する構成のままだと、入力の境界ファイルが新しい版に差し替わったり `queen = TRUE` の指定が変わったりといった変更が誰にも気づかれないまま下流の統計量にまで静かに伝わってしまい、比較の基準になる固定された成果物がリポジトリに残りません。このエッジ一覧は `scripts/build_geo.R` が二次医療圏の境界ポリゴンから `spdep::poly2nb(..., queen = TRUE)` で生成したもので、生成時の診断結果は `data/geo/adjacency_diagnostics.md` と `data/geo/README.md` にまとめてあります。
