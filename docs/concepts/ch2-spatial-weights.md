...
ここまでの説明で分析者が決めるのは「queen にするか rook にするか」というルールの選択であって、そのルールのもとでどの地域とどの地域が隣になるかは、境界ポリゴンの形状から機械的に決まります。ある2つの地域が境界線を共有しているか、queen の場合は頂点1点だけでも接しているか — これは幾何学的に判定できる事実であり、人間が地図を1地域ずつ見ながら「ここは隣、ここは隣でない」と判断していく作業ではありません。

Rでは、境界ポリゴンを `sf` オブジェクトとして読み込んであれば、隣接の判定は1行で済みます。

``` r
# polygons は sf オブジェクトとして読み込んだ境界ポリゴン
# (この教材の二次医療圏なら data/geo/iryoken2.geojson)
nb <- spdep::poly2nb(polygons, queen = TRUE)
```

`poly2nb()` が返す `nb` オブジェクトは、ハンズオンで組み立てているものと同じ種類のオブジェクトです。

ではなぜ本教材のハンズオンは、この1行をその場で呼ぶのではなく、あらかじめ計算済みの `data/geo/adjacency_iryoken2.csv`(列は `area_code` / `neighbor_code` のエッジ一覧)を読み込む構成になっているのでしょうか。理由は2つありますが、主な理由は教材としての方針です。隣接の定義をコミット済みの成果物として固定しておくことは、本章の主題である「隣を先に決める」という考え方をそのまま体現しています。

`poly2nb()` 自体は、同じポリゴンと同じオプションを渡せば常に同じ結果を返す決定的な関数です。それでも隣接をその場で計算する構成のままだと、入力の境界ファイルの版や `queen = TRUE` の指定が変わったとき、その変更が誰にも気づかれないまま下流の統計量まで静かに伝わります。そして、比較の基準になる固定された成果物もリポジトリに残りません。

このエッジ一覧は `scripts/build_geo.R` が二次医療圏の境界ポリゴンから `spdep::poly2nb(..., queen = TRUE)` で生成したもので、生成時の診断結果は `data/geo/adjacency_diagnostics.md` と `data/geo/README.md` にまとめてあります。

副次的な理由として、この教材を作成した環境では `spdep::poly2nb()` や `spdep::mat2listw()` を呼ぶと計算結果自体は正しく出るもののRプロセスが終了時に異常終了する、という事情もあります(この現象については[ハンズオン①](../handson/01-map-moran-lisa-gi.md)も参照)。読者の環境ではこの問題が起きない可能性は十分にありますが、起きない場合でも、隣接の定義を成果物として固定しておくこと自体は引き続き推奨される進め方です。
...
距離閾値もk近傍も、境界を共有しない地域(離島や飛び地)にも隣を割り当てられるという利点があります。この利点が実務上どれだけ重要かは、後の「定義を変えると結果は変わる」の節で見ます。
...
- どの地域が隣になるかは境界ポリゴンの形状から機械的に決まる。本教材はその隣接関係をコミット済みの成果物として固定している
