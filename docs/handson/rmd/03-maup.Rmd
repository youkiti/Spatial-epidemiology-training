---
title: "ハンズオン③: MAUPの実演 — 都道府県 vs 二次医療圏"
data_inputs:
  - data/geo/adjacency_iryoken2.csv
  - data/geo/adjacency_prefecture.csv
  - data/geo/iryoken2.geojson
  - data/geo/prefecture.geojson
...
# OS 標準搭載の日本語フォントを既定にする。00-setup.Rmd・01-map-moran-lisa-gi.Rmd
# と同じ処理。
...
都道府県のqueen contiguityでは、**`r paste(pref_df$pref_name[card(pref_nb) == 0], collapse = "と")`が隣接0件(孤立)**になります。いずれも海で隔てられているため地理的に妥当な結果ですが、隣が0個の地域があると `nb2listw()` の既定設定はエラーで止まります。この先すべての計算で `zero.policy = TRUE` を明示し、「隣が0件の地域はエラーにせず、空間ラグを0として扱う」という扱いにします。**孤立地域のLISA分類・Gi\* z値は、このページの方針として明示的にNAへ上書きします**(`zero.policy = TRUE` が自動でそうするわけではありません。理由は後の「隣接ゼロの地域をどう扱ったか」の節で改めてまとめます)。
...
二次医療圏のqueen contiguityでは`r sum(card(iryoken2_nb) == 0)`区域が孤立します(いずれも離島。[ハンズオン①](01-map-moran-lisa-gi.md)・`data/geo/README.md` で確認済み)。都道府県`r sum(card(pref_nb) == 0)`件・二次医療圏`r sum(card(iryoken2_nb) == 0)`件という孤立の数そのものが、地域単位を細かくするほど「境界を共有する隣が1つも無い」区域が増えることを表しています。
...
都道府県(`r nrow(pref_df)`)と二次医療圏(`r nrow(iryoken2_df)`)、それぞれの境界データ(`data/geo/prefecture.geojson` / `data/geo/iryoken2.geojson`)に系列Aの人口10万対専門医数(`rate_care`)を結合し、地図にします。**単位を変えたことによる見た目の違いだけを見せるため、色の基準(`limits`)を2枚の地図で共通にします。**
...
```{r map-pref-rate, fig.alt=sprintf("%d都道府県の人口10万対専門医数(系列A・診療の場のみ)の地図。%sが最も濃く、%sも比較的濃い(ただし人口10万対専門医数の高さとGi*のhot spot判定は一致するとは限らない。詳しくはStep 4)。色の基準は次の二次医療圏の地図と共通(0〜約%.1f)にしてあるため、二次医療圏の地図に比べると全体に薄く、色の差も小さく見える。", nrow(pref_df), pref_by_rate$pref_name[1], paste(pref_by_rate$pref_name[2:4], collapse = "・"), rate_care_limits[2]), fig.width=6.5, fig.height=6}
ggplot(pref_map) +
...
```{r map-iryoken2-rate, fig.alt=sprintf("%d二次医療圏の人口10万対専門医数(系列A・診療の場のみ)の地図。色の基準は上の都道府県地図と共通(0〜約%.1f)。%s内(%sなど)が最も濃く、都道府県地図では%sという1色に均されていた濃淡が、複数の医療圏にまたがる濃淡の差として見える。%s内も%sなど濃い医療圏と0人の離島医療圏が混在しており、都道府県地図の単一の色には現れない内部のばらつきがある。", nrow(iryoken2_df), rate_care_limits[2], top_iryoken2_pref, areas_in(top_iryoken2_pref), top_iryoken2_pref, pref_by_rate$pref_name[1], areas_in(pref_by_rate$pref_name[1])), fig.width=6.5, fig.height=6}
ggplot(iryoken2_map) +
...
2枚の地図を色の基準を揃えて比べると、**都道府県地図のほうが全体に薄く見えます**。二次医療圏の最大値(`r round(max(iryoken2_df$rate_care, na.rm = TRUE), 1)`、`r iryoken2_df$pref_name[which.max(iryoken2_df$rate_care)]``r iryoken2_df$area_name[which.max(iryoken2_df$rate_care)]`)が都道府県の最大値(`r round(max(pref_df$rate_care, na.rm = TRUE), 1)`、`r pref_df$pref_name[which.max(pref_df$rate_care)]`)より大きいため、共通の色の基準では都道府県側の濃淡の差が圧縮されます。これは章6でいう scale effect そのものです——**細かい単位で存在する極端な値は、粗い単位に集約すると周囲の値と混ざって平均化され、地図上の見た目の差が小さくなります。**
...
**このテーブルだけを見ると、MAUPは大きな問題に見えないかもしれません。** 系列A・B・Cのいずれでも、都道府県・二次医療圏のどちらの単位でもGlobal Moran's Iは`r round(min(moran_table$I), 2)`〜`r round(max(moran_table$I), 2)`程度の正の値で、`moran.test()` のp値もmoran.mcの疑似p値もすべて0.05を下回ります。**「地図全体として緩やかな正の空間的自己相関がある」という結論そのものは、単位を変えても分子の定義を変えてもひっくり返りません。** ただし都道府県のp値(`r formatC(moran_pref_A[["p_value"]], format = "g", digits = 3)`)は二次医療圏のp値(`r formatC(moran_iryoken2_A[["p_value"]], format = "g", digits = 3)`)よりずっと大きく、地域数が`r nrow(pref_df)`まで減ると検出力が下がることが分かります。**MAUPの効果が本当に効いてくるのは、この先のLISA・Gi\*という「どこが集まっているか」を地域単位で見る段階です。**
...
人口10万対専門医数が最も高いのは長崎県(`r round(pref_lisa_gi$rate_care[pref_lisa_gi$pref_name == "長崎県"], 2)`)ですが、**人口10万対専門医数で`r pref_lisa_gi$rank_care[pref_lisa_gi$pref_name == "東京都"]`位の東京都(`r round(pref_lisa_gi$rate_care[pref_lisa_gi$pref_name == "東京都"], 2)`)は、LISA分類が`r pref_lisa_gi$quadrant[pref_lisa_gi$pref_name == "東京都"]`(空間的アウトライヤー)、Gi\* z値も`r round(pref_lisa_gi$gi_z[pref_lisa_gi$pref_name == "東京都"], 3)`とhot spot閾値1.96を大きく下回ります。** 東京都に隣接する埼玉県・千葉県・神奈川県の人口10万対専門医数が東京都ほど高くないため、[ハンズオン①](01-map-moran-lisa-gi.md)のB市(架空10市町村)と同じ理由で、周囲が低い単独の高値として扱われます。

対照的に、**長崎県は`r pref_lisa_gi$quadrant[pref_lisa_gi$pref_name == "長崎県"]`、Gi\* z値`r round(pref_lisa_gi$gi_z[pref_lisa_gi$pref_name == "長崎県"], 3)`で、都道府県レベルでもhot spot閾値を超えます。** `r nagasaki_neighbor_text`
...
都道府県レベルでGi\*がhot spotとして検出するのは、この`r nrow(pref_hotspots)`県(`r paste(pref_hotspots$pref_name, collapse = "・")`)だけです。ここで疑問が生まれます——**「都道府県レベルでは目立たない東京都」は、二次医療圏まで単位を細かくしても本当に目立たないままなのでしょうか。** 「都道府県レベルでhot spotになった県」と「ならなかった東京都」を、それぞれ二次医療圏に割ったときの中身を見て確認します。
...
長崎県内では、`r paste(nagasaki_hh$area_name, collapse = "・")`の`r nrow(nagasaki_hh)`医療圏がHigh-Highに分類され、うち`r n_nagasaki_hh_hotspot`医療圏がGi\*でもhot spot閾値を超えます。局所p値0.05未満で絞ると、有意なHigh-Highは`r nrow(nagasaki_hh_sig)`医療圏(`r paste(nagasaki_hh_sig$area_name, collapse = "・")`)に減ります——**`r nagasaki_hh_nonsig$area_name`はHigh-Highに分類されてはいるものの、その分類自体が有意ではなく(局所p値`r round(nagasaki_hh_nonsig$local_p, 3)`)、ちょうどGi\*の閾値も下回っています**(z値`r round(nagasaki_hh_nonsig$gi_z, 3)`)。これは、後で東京都(Step 4後半)でも確認することになる同じ構造です——HH分類が有意でない唯一の医療圏が、ちょうどGi\*の閾値も下回ります。[ハンズオン①の「有意性についての注意」](01-map-moran-lisa-gi.md)がそのまま当てはまる例です。残る**`r paste(nagasaki_iso$area_name, collapse = "・")`の`r nrow(nagasaki_iso)`医療圏は隣接0件のため、LISA分類・Gi\*とも計算が成立せずNAになります**。`r paste(nagasaki_iso_zero$area_name, collapse = "・")`は人口10万対専門医数も0ですが、**`r nagasaki_iso_nonzero$area_name`は人口10万対専門医数`r round(nagasaki_iso_nonzero$rate_care, 2)`と長崎県内`r nrow(nagasaki)`医療圏中`r nagasaki_iso_rank`位の値でありながら、隣が1つも無いというだけの理由でLISA・Gi\*とも分類できません**——孤立地域の扱いが「人口10万対専門医数が低いから」ではなく「隣接の定義上たまたま孤立しているから」という、人口10万対専門医数の高低とは別の理由で効くことの一例です。**都道府県レベルの「長崎県はhot spot」という結論は、二次医療圏まで割っても崩れません** — 内部の主要医療圏がそろって高い値を示しているため、粗い単位でも細かい単位でも同じ結論になります。
...
**ここが今回の山場です。** 都道府県レベルでは空間的アウトライヤー(hot spotではない)と分類された東京都の内部を見ると、`r paste(head(arrange(tokyo, desc(rate_care))$area_name, 3), collapse = "・")`など都心部の複数の医療圏で高い値が観測されます。ここで**[ハンズオン①の「有意性についての注意」](01-map-moran-lisa-gi.md)を思い出してください**——LISAの4分類はそれ自体では有意性を含まないので、この東京都の表でも局所p値まで見て初めて何が言えるかが決まります。

LISAでHigh-Highに分類されるのは`r nrow(tokyo_highhigh)`医療圏、`r tokyo_count_text`。**ただしこれは局所p値を無視した無補正の一致であり、中身まで同じではありません。** 局所p値0.05未満で絞ると、有意なHigh-Highは`r nrow(tokyo_highhigh_sig)`医療圏に減ります。**この有意なHigh-High`r nrow(tokyo_highhigh_sig)`医療圏は、すべてGi\*でもhot spotと判定されています。** Gi\*はさらに`r gi_not_hh_row$area_name`(LISA分類は`r gi_not_hh_row$quadrant``r gi_not_hh_note`、局所p値`r round(gi_not_hh_row$local_p, 3)`)を1件多く拾います——LISAとGi\*は別の問いに対する別の答えなので、これは矛盾ではなく、Gi\*が「自分と周囲の関係の分類」ではなく「塊」という別の角度から検出した結果です。

唯一の「High-Highだがhot spotでない」医療圏である`r hh_not_gi_row$area_name`(Gi\* z値`r round(hh_not_gi_row$gi_z, 3)`)は、**そもそもHigh-High分類自体が有意ではありません**(局所p値`r round(hh_not_gi_row$local_p, 3)`)。つまりこれは「LISAはHigh-Highと言っているのにGi\*が否定した」例ではなく、**①が説明した『LISA分類は有意性を含まない』をそのまま体現した実例です。** それでもGi\*でhot spotと判定された`r nrow(tokyo_gi_hotspots)`医療圏はいずれも都心部に集まっており、z値の最大は`r tokyo$area_name[1]`のz = `r round(tokyo$gi_z[1], 2)`です。**都道府県という単位では「東京都1件」として埼玉県・千葉県・神奈川県という比較的人口10万対専門医数の低い隣県と平均され、周囲より浮いた単独の高値に見えていましたが、二次医療圏まで単位を細かくすると、都心部の複数の医療圏がGi\*でもhot spotとして検出されることが分かります。**
...
```{r map-gi-pref, fig.alt=sprintf("%d都道府県のGi* z値をhot spot・非有意・孤立(NA)の3区分で塗った地図(cold spotは0件のため凡例に出ない)。深い赤(hot spot)は%sの%d県だけで、%d都道府県は明るいグレー(非有意)。%sの%d都道府県は隣接0件のため中間の濃さのグレー(孤立(NA))で塗られ、色に加えて破線の輪郭を重ねて区別できるようにしてある。", nrow(pref_df), hot_pref_names, n_hot_pref, n_nonsig_pref, isolated_pref_names, n_isolated_pref), fig.width=6.5, fig.height=6}
ggplot(pref_gi_map) +
...
```{r map-gi-iryoken2, fig.alt=sprintf("%d二次医療圏のGi* z値を上と同じ3区分・同じ配色で塗った地図(cold spotは0件のため凡例に出ない)。深い赤(hot spot)は%s、あわせて%d医療圏に散らばる(いずれも無補正のz > 1.96による件数)。都道府県地図では明るいグレー(非有意)だった%sの一部医療圏もここでは赤くなる。離島など隣接0件の%d区域は中間の濃さのグレー(孤立(NA))で、上の都道府県地図と同じく破線の輪郭を重ねてある。", nrow(iryoken2_df), hot_iryoken2_areas_text, n_hot_iryoken2, newly_hot_pref_text, n_isolated_iryoken2), fig.width=6.5, fig.height=6}
...
都道府県地図で赤くなるのは`r hot_pref_names`の`r n_hot_pref`県だけですが、二次医療圏地図では`r newly_hot_pref_text`にも赤が現れ、hot spotと判定される医療圏の総数は`r n_hot_iryoken2`まで増えます(**無補正のz > 1.96で数えた件数で、多重比較の補正はしていません。339地域を同時に検定するとp値の読み方が変わることは[ハンズオン①の多重比較の注意](01-map-moran-lisa-gi.md)で確認した通りです**)。この増加には2つの効果が重なっています——**Step 2で見た「集約による平滑化」が解け、都心部などの局所的な高値が周囲と混ざらずに残ること**(scale effectそのもの)に加えて、**地域数が`r nrow(pref_df)`から`r nrow(iryoken2_df)`へ増えることでGi\*の検定力自体が上がること**(Step 3で確認した「地域数が少ないと検出力が下がる」効果の裏返し)です。**都道府県地図には無かった赤が、二次医療圏地図には現れる**——これが表で確認した東京都の分類の入れ替わりを、地図としても裏付けています。なお両方の地図とも `cold spot`(周囲を含めて低い値の塊)は1件も検出されませんでした——この専門医偏在のデータでは、「集中して多い」場所はあっても「集中して少ない」場所は(この閾値では)無かったということです。
...
都道府県レベルで見ると、queen contiguityのMoran's I(`r round(ws_pref_queen[["I"]], 3)`)に対して、距離閾値300kmでは`r round(ws_pref_dn300[["I"]], 3)`、孤立を完全に解消する800kmでは`r round(ws_pref_dn800[["I"]], 3)`まで下がります。800kmまで緩めると`r nrow(pref_df)`都道府県のほとんどが互いに隣接する密なグラフになり(平均隣接数が跳ね上がります)、「隣」というより「ほぼ全国が均等に影響し合う」設定に近づくため、空間的な集中の情報が薄まってI自体が小さくなります。一方でk近傍(k=4)は孤立を解消しながらもqueenに近い`r round(ws_pref_kn4[["I"]], 3)`という値になりました。**「孤立を解消する」という同じ目的でも、距離閾値をどこまで緩めるかによってMoran's Iの値そのものが数倍単位で変わります。** これが[章6](../concepts/ch6-pitfalls.md)の5番目の落とし穴「『隣』の定義の事後決定」の実例です——分析後に都合の良い定義を選べてしまう余地が、これだけの幅で存在します。
...
このページを通じて、都道府県`r sum(pref_lisa_gi$isolated)`件(`r isolated_pref_names`)・二次医療圏`r sum(iryoken2_lisa_gi$isolated)`件(いずれも離島)が queen contiguity で孤立します。`moran.test()` / `localmoran()` / `localG()` はいずれも `zero.policy = TRUE` を渡すことで、孤立地域をエラーにせず「隣が無い地域」として計算を続行します。具体的には、空間ラグ(`lag.listw()` が計算する「隣の値の平均」)は孤立地域では定義できないため0として扱われますが、**「周囲と比べてどうか」という分類そのものが孤立地域には定義できないため、このページではStep 4で `isolated <- card(nb) == 0` を使って、孤立地域のLISA分類(`quadrant`)とGi\* z値(`gi_z`)を明示的にNAへ上書きしています。** `zero.policy = TRUE` でエラーを止めることと、孤立地域の局所統計を意味のある「結果」として見せないことは別の配慮です。

次の表の通り、孤立地域はいずれもLISA分類・局所p値・Gi\* z値がNA(またはNaN)になります。

```{r isolated-regions-table}
pref_lisa_gi |>
  filter(isolated) |>
  dplyr::select(pref_name, rate_care, quadrant, local_p, gi_z) |>
  knitr::kable(digits = 3, col.names = c("都道府県", "人口10万対専門医数", "LISA分類", "局所p値", "Gi* z値"))
...
```{r isolated-regions-table-iryoken2}
iryoken2_lisa_gi |>
  filter(isolated) |>
...
```{r isolated-effect-on-global-moran}
...
孤立地域を隣接から除外しているわけではありませんが、Global Moran's Iの計算対象にもそのまま残っているわけではありません。`zero.policy = TRUE` は孤立地域があってもエラーにせず計算を続行させるだけの設定で、`moran.test()` の既定である `adjust.n = TRUE` が、隣接0件の地域をnからも落とします。実際、都道府県のnは`r n_pref_all`ではなく`r n_pref_used`、二次医療圏のnは`r n_iryoken2_all`ではなく`r n_iryoken2_used`で計算されています。この違いはMoran's Iの値そのものにも表れます——二次医療圏のMoran's I(系列A)は既定(`adjust.n = TRUE`、Step 3の表の値)では`r round(I_iryoken2_adjn, 4)`ですが、孤立地域も含めた`r n_iryoken2_all`区域をnとして数える`adjust.n = FALSE`では`r round(I_iryoken2_noadjn, 4)`に変わります。

前のStep 5で見た通り、k近傍や十分に広い距離閾値に切り替えれば孤立を解消できます(都道府県の800km・k近傍(k=4)、二次医療圏のk近傍(k=6)はいずれも孤立0件でした)。ただし**距離閾値が狭すぎるとむしろ孤立が増えることもあります**——実際、二次医療圏の距離閾値50kmでは、queen contiguityの`r n_isolated(iryoken2_nb)`件から`r n_isolated(iryoken2_dn50)`件へ孤立が増えました。いずれの場合も、その代わりにMoran's Iの値が変わります。**「孤立を解消する」ことと「隣の定義を変えずに孤立を許容する」ことのどちらが良いかにも、唯一の正解はありません。** 本ページでは主要な結果(Step 2〜4)はqueen contiguity + zero.policyのまま示し、距離閾値・k近傍はあくまで感度分析として別枠で扱いました。
