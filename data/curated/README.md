...
推論である以上、根拠を残すだけでは足りず、**誰がその推論を妥当と判断したか**が要る。

- **2026-08-19、本教材の著者(医師)が全138行を目視監査し、承認した。**
  監査の対象は (1) 大学名だけの記載をその大学の医学部附属病院に対応づける判断、
  (2) `care_setting` の判定(診療の場かどうか)の2点。
- この監査の過程で著者から「国立健康危機管理研究機構 国立感染症研究所は診療機関ではない」
  という指摘があり、`care_setting` を導入して2系列を出す設計に変わった
  (上記「`care_setting`(診療の場かどうか)」)。
- 名簿PDFが改訂されてこの表を作り直すときは、**承認も取り直すこと。**
  前回の承認は前回の138行に対するものであって、新しい行には及ばない。
