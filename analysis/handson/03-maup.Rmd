...
都道府県のqueen contiguityでは、**`r paste(pref_df$pref_name[card(pref_nb) == 0], collapse = "と")`が隣接0件(孤立)**になります。いずれも海で隔てられているため地理的に妥当な結果ですが、隣が0個の地域があると `nb2listw()` の既定設定はエラーで止まります。この先すべての計算で `zero.policy = TRUE` を明示し、「隣が0件の地域はエラーにせず、空間ラグを0として扱う(局所統計はNAになる)」という扱いにします(理由は後の「隣接ゼロの地域をどう扱ったか」の節で改めてまとめます)。
...
**ここが今回の山場です。** 都道府県レベルでは空間的アウトライヤー(hot spotではない)と分類された東京都の内部を見ると、`r paste(head(arrange(tokyo, desc(rate_care))$area_name, 3), collapse = "・")`など都心部の複数の医療圏で高い値が観測されます。ただし**Gi\*とLISAは同じ集合を指しているわけではありません**——LISAでHigh-Highに分類されるのは`r nrow(tokyo_highhigh)`医療圏、`r tokyo_count_text`。`r gi_not_hh_row$area_name`はLISA分類が`r gi_not_hh_row$quadrant``r gi_not_hh_note`であるにもかかわらずGi\* z値は`r round(gi_not_hh_row$gi_z, 3)`でhot spot閾値(1.96)を超え、逆に`r hh_not_gi_row$area_name`はHigh-Highに分類されながらGi\* z値は`r round(hh_not_gi_row$gi_z, 3)`にとどまり閾値を超えません。**「値が高い(自分と周囲がそろってHigh-High)」ことと「Gi\*でhot spotと判定される」ことは、別の問いに対する別の答えです。** それでもGi\*でhot spotと判定された`r nrow(tokyo_gi_hotspots)`医療圏はいずれも都心部に集まっており、z値の最大は`r tokyo$area_name[1]`のz = `r round(tokyo$gi_z[1], 2)`です。**都道府県という単位では「東京都1件」として埼玉県・千葉県・神奈川県という比較的率の低い隣県と平均され、周囲より浮いた単独の高値に見えていましたが、二次医療圏まで単位を細かくすると、都心部の複数の医療圏がGi\*でもhot spotとして検出されることが分かります。**
...
```{r map-gi-pref, fig.alt=sprintf("%d都道府県のGi* z値をhot spot・非有意・孤立(NA)の3区分で塗った地図(cold spotは0件のため凡例に出ない)。赤(hot spot)は%sの%d県だけで、%d都道府県は非有意(グレー)。%sの%d都道府県は隣接0件のため孤立(NA、濃いグレー)。", nrow(pref_df), hot_pref_names, n_hot_pref, n_nonsig_pref, isolated_pref_names, n_isolated_pref), fig.width=6.5, fig.height=6}
ggplot(pref_gi_map) +
...
```{r map-gi-iryoken2, fig.alt=sprintf("%d二次医療圏のGi* z値を上と同じ3区分・同じ配色で塗った地図(cold spotは0件のため凡例に出ない)。赤(hot spot)は%s、あわせて%d医療圏に散らばる。都道府県地図では非有意だった%sの一部医療圏もここでは赤くなる。離島など隣接0件の%d区域は孤立(NA)。", nrow(iryoken2_df), hot_iryoken2_areas_text, n_hot_iryoken2, newly_hot_pref_text, n_isolated_iryoken2), fig.width=6.5, fig.height=6}
...
都道府県地図で赤くなるのは`r hot_pref_names`の`r n_hot_pref`県だけですが、二次医療圏地図では`r newly_hot_pref_text`にも赤が現れ、hot spotと判定される医療圏の総数は`r n_hot_iryoken2`まで増えます。**都道府県地図には無かった赤が、二次医療圏地図には現れる**——これが表で確認した東京都の分類の入れ替わりを、地図としても裏付けています。なお両方の地図とも `cold spot`(周囲を含めて低い値の塊)は1件も検出されませんでした——この専門医偏在のデータでは、「集中して多い」場所はあっても「集中して少ない」場所は(この閾値では)無かったということです。
