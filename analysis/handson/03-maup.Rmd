...
```{r map-pref-rate, fig.alt=sprintf("%d都道府県の人口10万対専門医数(系列A・診療の場のみ)の地図。%sが最も濃く、%sも比較的濃い(ただし率の高さとGi*のhot spot判定は一致するとは限らない。詳しくはStep 4)。色の基準は次の二次医療圏の地図と共通(0〜約%.1f)にしてあるため、二次医療圏の地図に比べると全体に薄く、色の差も小さく見える。", nrow(pref_df), pref_by_rate$pref_name[1], paste(pref_by_rate$pref_name[2:4], collapse = "・"), rate_care_limits[2]), fig.width=6.5, fig.height=6}
ggplot(pref_map) +
...
率が最も高いのは長崎県(`r round(pref_lisa_gi$rate_care[pref_lisa_gi$pref_name == "長崎県"], 2)`)ですが、**率で`r pref_lisa_gi$rank_care[pref_lisa_gi$pref_name == "東京都"]`位の東京都(`r round(pref_lisa_gi$rate_care[pref_lisa_gi$pref_name == "東京都"], 2)`)は、LISA分類が`r pref_lisa_gi$quadrant[pref_lisa_gi$pref_name == "東京都"]`(空間的アウトライヤー)、Gi\* z値も`r round(pref_lisa_gi$gi_z[pref_lisa_gi$pref_name == "東京都"], 3)`とhot spot閾値1.96を大きく下回ります。** 東京都に隣接する埼玉県・千葉県・神奈川県の率が東京都ほど高くないため、[ハンズオン①](01-map-moran-lisa-gi.md)のB市(架空10市町村)と同じ理由で、周囲が低い単独の高値として扱われます。
